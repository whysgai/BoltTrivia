...
const WaitingScreen = () => {
  ...
  const endCondition = useSelector(
    (state) => state.gameStateReducer.endCondition
  );

  return (
    <>
      {gamePhase === GAME_PHASE.SELECT_GAME_TYPE && playerNumber === 1 ? (
        <p>Waiting for Host to select game mode</p>
      ...
      ) : gamePhase === GAME_PHASE.AWAITING_RESULTS &&
        endCondition === END_CONDITION.OUT_OF_QUESTIONS ? (
        <p>No more questions: waiting for other player to finish</p>
      ) : gamePhase === GAME_PHASE.AWAITING_RESULTS &&
        endCondition === END_CONDITION.OUT_OF_TIME ? (
        <p>Out of time: processing results</p>
      ) : gamePhase === GAME_PHASE.AWAITING_RESULTS &&
        endCondition === END_CONDITION.SCORE_REACHED ? (
        <p>Goal reached: processing results</p>
      ) : gamePhase === GAME_PHASE.AWAITING_RESULTS &&
        endCondition === END_CONDITION.OTHER_SCORE_REACHED ? (
